...
def ButtonChoose(message):
    ...
    if message.text == cursorConnectionAndRead("SELECT text FROM buttons WHERE id=10")[0][0]:
        Vacancies(message)
    if message.text == cursorConnectionAndRead("SELECT text FROM buttons WHERE id=15")[0][0]:
        print("1")
    
def requests(message):
    global results
    bot.send_message(message.chat.id, cursorConnectionAndRead("SELECT text FROM greetings WHERE id=2")[0][0]+"\n"+
    str("1")+"."+cursorConnectionAndRead("SELECT text FROM Favorite_problems WHERE id=1")[0][0]+"\n"+
    str("2")+"."+cursorConnectionAndRead("SELECT text FROM Favorite_problems WHERE id=2")[0][0]+"\n"+
    str("3")+"."+cursorConnectionAndRead("SELECT text FROM Favorite_problems WHERE id=3")[0][0]+"\n"+
    str("4")+"."+cursorConnectionAndRead("SELECT text FROM buttons WHERE id=14")[0][0]+"\n"+
    str("5")+"."+cursorConnectionAndRead("SELECT text FROM buttons WHERE id=15")[0][0])
    bot.register_next_step_handler(message, ChooseProblems)
def ChooseProblems(message):
    if message.text == "1":
        RegisterProblems(message)
    if message.text == "2":
        RegisterProblems(message)
    if message.text == "3":
        RegisterProblems(message)
    if message.text == "4":
        Fullproblems(message)
    if message.text == "5":
        Favoriteproblems+(message)
def RegisterProblems(message):
    global results
    global myArray
    ...
    if results !=[]:
        for i in results:
            bot.send_message(message.chat.id,i)
            bot.register_next_step_handler(message,RegisterProblemsStep2)
            del results[0]
            break
    else:
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True,one_time_keyboard=True) #one_time_keyboard=True - спрятать кнопку после нажатия
        ...
        bot.register_next_step_handler(message,finalStepOfRegistrProblems)
def Registerproblems(message):
    global results
    global myArray
    ...
    print(results)
        ...
        for i in results:
            ...
            del results[0]
            break
            bot.register_next_step_handler(message,finalStepOfRegistrationProblems)
def RegisterProblemsStep2(message):
    myArray.append(message.text)
    print(myArray)
    
    Registerproblems(message)
def finalStepOfRegistrationProblems(message):
    global myArray

    if message.text == "Отправить":
        bot.send_message(message.chat.id,
                         'Ваша проблема будет решена в ближайшее время'+"\n"
                         +'если вы написали что то не правильно сбросьте командой /start')
...
def Fullproblems(message):
    ...
    str(2)+"."+cursorConnectionAndRead("SELECT text FROM problems WHERE id=2")[0][0]+"\n"+
    str(3)+"."+cursorConnectionAndRead("SELECT text FROM problems WHERE id=3")[0][0]+"\n"+
    str(4)+"."+cursorConnectionAndRead("SELECT text FROM problems WHERE id=4")[0][0]+"\n"+
    str(5)+"."+cursorConnectionAndRead("SELECT text FROM problems WHERE id=5")[0][0]+"\n"+
    str(6)+"."+cursorConnectionAndRead("SELECT text FROM problems WHERE id=6")[0][0]+"\n"+
    str(7)+"."+cursorConnectionAndRead("SELECT text FROM problems WHERE id=7")[0][0]+"\n"+
    str(8)+"."+cursorConnectionAndRead("SELECT text FROM problems WHERE id=8")[0][0]+"\n"+
    str(9)+"."+cursorConnectionAndRead("SELECT text FROM problems WHERE id=9")[0][0]+"\n"+
    str(10)+"."+cursorConnectionAndRead("SELECT text FROM problems WHERE id=10")[0][0])
    bot.register_next_step_handler(message, ChooseProblemsFull)
def ChooseProblemsFull(message):
    if message.text == "1":
        cursorConnectionAndWrite()
        RegisterProblems(message)
    if message.text == "2":
        RegisterProblems(message)
    if message.text == "3":
